  ...
  void SparseLU<MatrixType, OrderingType>::factorize(const MatrixType &matrix) {
    ...
    for(jcol = 0; jcol < n;) {
      ...
      for(jj = jcol; jj < jcol + panel_size; jj++) {
        ...
        info = Base::column_dfs(m, jj, m_perm_r.indices(), m_perfv.maxsuper, nseg, panel_lsubk, segrep, repfnz_k,
    ...
    void solveInPlace(MatrixBase <Dest> &X) const {
      ...
      for(Index k = m_mapL.nsuper(); k >= 0; k--) {
        ...
        if(nsupc == 1) {
          ...
          Map<const Matrix <Scalar, Dynamic, Dynamic, ColMajor>, 0, OuterStride<> > A(&(m_mapL.valuePtr()[luptr]),
                                                                                      nsupc, nsupc, OuterStride<>(lda));
