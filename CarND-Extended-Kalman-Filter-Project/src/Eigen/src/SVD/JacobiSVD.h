    ...
    struct svd_precondition_2x2_block_to_be_real<MatrixType, QRPreconditioner, true> {
      ...
      static bool run(typename SVD::WorkMatrixType &work_matrix, SVD &svd, Index p, Index q, RealScalar &maxDiagEntry) {
        ...
        maxDiagEntry = max EIGEN_EMPTY(maxDiagEntry,
                                       max EIGEN_EMPTY(abs(work_matrix.coeff(p, p)), abs(work_matrix.coeff(q, q))));
      ...
      for(Index p = 1; p < m_diagSize; ++p) {
        for(Index q = 0; q < p; ++q) {
          ...
          if(abs(m_workMatrix.coeff(p, q)) > threshold || abs(m_workMatrix.coeff(q, p)) > threshold) {
            ...
            if(internal::svd_precondition_2x2_block_to_be_real<MatrixType, QRPreconditioner>::run(m_workMatrix, *this,
                                                                                                  p, q, maxDiagEntry)) {
